const conectanomysqlAPI = require("../funcoes/database");
...
module.exports = {
  async listaTodosUsuarios(req, res) {
    const users = await conectanomysqlAPI.query(
      `
              SELECT id, cpf, nome, username, salt FROM Users
              `,
      { type: conectanomysqlAPI.QueryTypes.SELECT }
    );
  ...
  async cadastraUsuario(req, res) {
    ...
    const user = await conectanomysqlAPI.query(
      `
        INSERT INTO Users
  ...
  async verificasenha(req, res) {
    ...
    var user = await conectanomysqlAPI.query(
      `
